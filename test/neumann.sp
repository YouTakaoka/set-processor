...
def next: Set -> Set; s ->
    if is_neumann(s)
        then {from_n(s)}
            ...
            let t = get_target(s) |
            if t in s then s - {t} + {next(t)} else s + {t}
        )
...
def get_target: Set -> Set; s ->
    if (let m = get_max(s) | !is_neumann(m))
        then m
        else _get_target(s - {m}, m)
def _get_target: Set, Set -> Set; s, u ->
    if (let m = get_max(s) | is_neumann(m) && succ(m) == u)
        then _get_target(s - {m}, m)
        else m

% sの要素の中で最大の集合を見つける s: ターゲット集合, t, u: 探索集合
% sが空なら{}が返る．sが{{}}でも{}が返る．
def get_max: Set -> Set; s -> _get_max(s, {}, {})
def _get_max: Set, Set, Set -> Set; s, r, u ->
    if r == s
        then u
        else (
            let w = if r in s then r else u |
            _get_max(s, next(r), w)
        )
